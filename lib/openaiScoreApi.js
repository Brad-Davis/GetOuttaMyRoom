 ...
 */
async function invokeOpenAi(apiKey, systemInstructions, userPrompt) {
    let sys = systemInstructions;
    if (!/json/i.test(sys) && !/json/i.test(userPrompt)) {
        sys += ' Respond using json only matching the requested shape.';
    }
...
function buildScoreRequest(task, body) {
    ...
    const scoreJsonSystem = [
        ...
        '"score" must be a single number between 0 and 100 inclusive (floats ok if justified).',
    ].join(' ');
    ...
    if (task === 'linkedin') {
        return {
            ...
            systemInstructions:
                `${scoreJsonSystem} ` +
                `You score how highly you would think of a person that posts this on LinkedIn based on this snippet of text.`,
            userPrompt: trimmed,
        };
    }
    ...
    if (task === 'podcast') {
        ...
        return {
            ...
            systemInstructions:
                `${scoreJsonSystem} ` +
                `You score how likely you would listen to a podcast about ${topic} based on this snippet of text.`,
            userPrompt: trimmed,
        };
    }
    ...
    if (task === 'musictaste') {
        return {
            ...
            systemInstructions:
                `${scoreJsonSystem} ` +
                `You score how disappointed a traditional, judgmental family would be if their child passionately defended loving the supplied musician, based only on this snippet of what they said.`,
            userPrompt: trimmed,
        };
    }
    ...
    if (task === 'insult') {
        ...
        return {
            ...
            systemInstructions:
                `${scoreJsonSystem} ` +
                `You score how mean this insult is versus what it was supposed to be about (${insultTopic}), based only on this snippet of text.`,
            userPrompt: trimmed,
        };
    }
...
async function serveScore(req, res) {
    ...
    const { userPrompt, systemInstructions } = builtAny;

    try {
        const result = await invokeOpenAi(apiKey, systemInstructions, userPrompt);
