 ...
 * Prompts stay server-side; client only sends { task, text, topic?, question?, insultTopic? }.
 */
...
function buildScoreRequest(task, body) {
    ...
    if (task === 'linkedin') {
        return {
            ok: true,
            systemInstructions: `LinkedIn corporate impressiveness 0-100. Be lenient and encouraging: any genuine post (even cringe or humble-brag) scores 50+; typical effort 60-80; only empty, gibberish, or obvious spam below 35. ${JSON_SCORE_WITH_REASON}`,
            userPrompt: trimmed,
            maxTokens: 64,
        };
    }
    ...
    if (task === 'youtube') {
        return {
            ok: true,
            systemInstructions: `Youtube comment toxicity 0-100 (0=wholesome, 100=extremely toxic). ${JSON_SCORE_WITH_REASON}`,
            userPrompt: trimmed,
            maxTokens: 64,
        };
    }
    ...
    if (task === 'political') {
        const question =
            typeof body.question === 'string' ? body.question.trim().slice(0, MAX_TOPIC) : '';
        if (!question) {
            return { ok: false, status: 400, message: 'Missing question for political task' };
        }
        return {
            ok: true,
            systemInstructions: `Grade spoken answer to: "${question}" 0-100. 0=empty/nonsense/off-topic; 100=correct real answer; partial for wrong but on-topic attempts. ${JSON_SCORE_ONLY}`,
            userPrompt: trimmed,
            maxTokens: 12,
        };
    }
